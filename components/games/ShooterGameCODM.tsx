...
export default function ShooterGameCODM() {
        ...
        <div className="max-w-md w-full mx-4">
          <div className="bg-gray-800/90 backdrop-blur-sm border-2 border-blue-500/50 rounded-3xl p-8 shadow-2xl">
            ...
            <button
              ...
              className="w-full py-3 mt-4 text-gray-400 hover:text-white transition-colors flex items-center justify-center gap-2"
            >
              <ArrowLeft size={16} />
              Back to Games
            </button>
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center">
        <div className="max-w-2xl w-full mx-4">
          <div className="bg-gray-800/90 backdrop-blur-sm border-2 border-green-500/50 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="bg-gray-900/50 rounded-2xl p-6 mb-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-3xl font-mono font-bold text-green-400">{roomCode}</span>
                  <button
                    onClick={copyCode}
                    className="p-2 hover:bg-gray-700 rounded-lg transition-colors"
                  >
                    {copied ? <Check className="text-green-400" size={20} /> : <Copy className="text-gray-400" size={20} />}
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="text-white ml-2 font-semibold">{matchSettings.mode}</span>
                </div>
                <div>
                  <span className="text-gray-400">Max Players:</span>
                  <span className="text-white ml-2 font-semibold">{matchSettings.maxPlayers}</span>
                </div>
                <div>
                  <span className="text-gray-400">Score Limit:</span>
                  <span className="text-white ml-2 font-semibold">{matchSettings.scoreLimit}</span>
                </div>
                <div>
                  <span className="text-gray-400">Time:</span>
                  <span className="text-white ml-2 font-semibold">{matchSettings.timeLimit / 60} min</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-900/50 rounded-2xl p-6 mb-6">
              <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                <Users size={20} />
                Players ({otherPlayers.length + 1}/{matchSettings.maxPlayers})
              </h3>
              ...
              <div className="space-y-2">
                {localPlayer && (
                  <div className="flex items-center justify-between bg-gray-800 p-3 rounded-lg">
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${localPlayer.team === 'blue' ? 'bg-blue-500' : 'bg-red-500'}`}></div>
                      <span className="text-white font-semibold">{localPlayer.name}</span>
                      {multiplayer.getIsHost() && (
                ...
                {otherPlayers.map(player => (
                  <div key={player.id} className="flex items-center justify-between bg-gray-800 p-3 rounded-lg">
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${player.team === 'blue' ? 'bg-blue-500' : 'bg-red-500'}`}></div>
                ...
                {Array.from({ length: matchSettings.maxPlayers - otherPlayers.length - 1 }).map((_, i) => (
                  <div key={`empty-${i}`} className="flex items-center justify-between bg-gray-800/30 p-3 rounded-lg border-2 border-dashed border-gray-700">
            ...
            <div className="flex gap-4">
              {multiplayer.getIsHost() ? (
                <button
                  ...
                  className="flex-1 py-4 bg-gradient-to-r from-green-600 to-green-500 text-white rounded-xl font-bold text-lg hover:from-green-500 hover:to-green-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  START MATCH
                </button>
              ) : (
                <div className="flex-1 py-4 bg-yellow-500/20 border-2 border-yellow-500 text-yellow-500 rounded-xl font-bold text-lg text-center">
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col">
      ...
      <div className="absolute left-4 bottom-32 flex flex-col gap-3">
        <button
          onClick={() => {
            if (localPlayer) {
              setLocalPlayer(prev => prev ? { ...prev, isCrouching: !prev.isCrouching } : null);
            }
          }}
          className="w-14 h-14 bg-gray-800/80 backdrop-blur-sm border-2 border-white/30 rounded-full flex items-center justify-center text-white font-bold hover:bg-gray-700 transition-all shadow-lg"
        >
          ↓
        </button>
        ...
        <button
          onClick={startReload}
          className="w-14 h-14 bg-gray-800/80 backdrop-blur-sm border-2 border-yellow-500/50 rounded-full flex items-center justify-center text-yellow-400 font-bold hover:bg-gray-700 transition-all shadow-lg"
        >
          R
        </button>
      </div>

      {/* Killstreak Buttons (Right side) */}
      {killstreak >= 3 && (
        <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex flex-col gap-3">
          <button
            disabled={killstreak < 3}
            className="w-14 h-14 bg-blue-600/80 backdrop-blur-sm border-2 border-white/30 rounded-full flex items-center justify-center text-white text-2xl hover:bg-blue-500 transition-all shadow-lg disabled:opacity-30"
          >
            <Radio size={24} />
          </button>

          <button
            disabled={killstreak < 5}
            className="w-14 h-14 bg-red-600/80 backdrop-blur-sm border-2 border-white/30 rounded-full flex items-center justify-center text-white text-2xl hover:bg-red-500 transition-all shadow-lg disabled:opacity-30"
          >
            <Target size={24} />
          </button>

          <button
            disabled={killstreak < 7}
            className="w-14 h-14 bg-purple-600/80 backdrop-blur-sm border-2 border-white/30 rounded-full flex items-center justify-center text-white text-2xl hover:bg-purple-500 transition-all shadow-lg disabled:opacity-30"
          >
            <Zap size={24} />
          </button>
        </div>
      ...
      <button
        ...
        className="absolute top-4 left-4 w-10 h-10 bg-gray-800/80 backdrop-blur-sm border-2 border-white/30 rounded-lg flex items-center justify-center text-white hover:bg-gray-700 transition-all shadow-lg z-50"
      >
        ⏸
      </button>
      ...
      {/* Team indicator */}
      {localPlayer && (
        <div className="absolute top-4 right-4 flex items-center gap-2 bg-gray-800/80 backdrop-blur-sm px-4 py-2 rounded-lg border-2 border-white/30 shadow-lg">
          <div className={`w-3 h-3 rounded-full ${localPlayer.team === 'blue' ? 'bg-blue-500' : 'bg-red-500'}`}></div>
          <span className="text-white font-bold text-sm">{localPlayer.team.toUpperCase()} TEAM</span>
        </div>
      )}
    </div>
  );
